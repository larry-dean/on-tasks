...
//        })
    ...
    DellWsmanProcessNodesJob.prototype.processNode = function() {
        ...
        .then(function(node) {
            ...
            if (self.redfishUDM && node) {
                ...
                return Promise.props({
                        hardware: self.dataFactory(node.id, 'hardware'), 
                        chassis: self.dataFactory(node.id, 'chassis'), 
                        boot: self.dataFactory(node.id, 'boot'),
                        secureBoot: true,
                        obm: [node.id, 'RackHD'],
                        systemType: "Physical"
                        }); 
        ...
        .then(function(data) {
            logger.info("LDVP ************************ data here"+ JSON.stringify(data, null, 4));
    ...
    DellWsmanProcessNodesJob.prototype.updateCache = function (data) {
 
        var self = this;
        var ipAddr = "localhost"
        var port = "8080"
        var host = 'http://' + ipAddr + ':' + port 
        var path = '/redfish/v1/Systems';
        var method = 'POST';
      
        logger.info('IP Range discovery submitted...');
        return self.clientRequest(host, path, method, data)
        .then(function(result) {
            logger.info("**** client request result: " + JSON.stringify(result, null, 4));
            return result;
        })
        .catch(function(err) {
    ...
    DellWsmanProcessNodesJob.prototype.clientRequest = function(host, path, method, data) {
        ...
        logger.info("********LDBH path: " + setups.url.path);
        logger.info("********LDBH method: " + setups.method );
        logger.info("********LDBH host: " + setups.url.host );
        logger.info("********LDBH port: " + setups.url.port );
